...
boost::mutex WP_permission_mutex;
void WP_permission_callback(const std_msgs::Int8 WP_permission_msg)
{
	WP_permission_mutex.lock();
		path_wp_permission = WP_permission_msg.data;
		ROS_INFO( " !!! RECEIVED NEW WP_permission : %d " , path_wp_permission );
    WP_permission_mutex.unlock();
}
...
// wp_driver calculating the required bobcat acceleration and stering commands in order to get to a WP, and publishing them on ROS topic. 
     ...
     int mission_status = 0; // 0 - waiting for commands   , 1-going for the next WP
...
//     ROS_INFO(" bobcat_vel_azi = %f , (bobcat_azi-jb*2*PI) = %f , jb = %d , relative_vel_der = %f", bobcat_vel_azi , (bobcat_azi-jb*2*PI) , jb, relative_vel_der/PI);
          ...
          {      
               if ( ((wp+1) <= wp_num) || (bypass_on == 1)  )
                 { 
                   ...
                   if (bypass_on == 1)   // reaching bypass WP
                    { 
                       bypass_on = 0;
                       ROS_INFO("Reache bypass WP");      
                    }
                   else if (bypass_on == 0) // reaching path WP
                    {
                	   mission_status = wp;
                	   WP_permission_mutex.lock();
                       if ((wp+1) <= path_wp_permission)
                       	   {
                    	   	   wp = wp + 1;
                    	   	   ROS_INFO("Reached wp %d of %d Permitted out of %d in total !!! going for to the next..." , wp , path_wp_permission, wp_num );
                       	   }
                       	   else
                       	   {
                       		   ROS_INFO("Reached wp %d of %d Permitted out of %d in total !!! wating ... " , wp , path_wp_permission, wp_num );
                       	   }
                      WP_permission_mutex.unlock();
                    }
    ...
    // mission status publishing
		  std_msgs::Int8 mission_status_msg;
...
int main(int argc, char **argv)
{
  	...
  	std::string robNS = argv[1];

	ros::init(argc, argv, robNS+"_srvss_wp_driver_node");
  	robNS= "/"+ robNS;
 
    if ( (std::string(argv[2]).compare("-path")!=0) && (std::string(argv[2]).compare("-file")!=0) )
    {
    	std::cout << "111usage:" <<std:: endl;
    ...
    else if (std::string(argv[2]).compare("-path")==0)
		{
...
*/
	      ...
	      for (int i=1 ; i<=wp_num ; i++)
		    {
		      t_lat[i] = std::atof(argv[3+3*(i-1)]);
		      t_lon[i] = std::atof(argv[4+3*(i-1)]);
		      t_vel[i] = std::atof(argv[5+3*(i-1)]);
		    }
		}

  else if(std::string(argv[2]).compare("-file")==0)
		{
  ...
  ros::Subscriber WP_command = n.subscribe(robNS+"/WP_command", 100, WP_callback);
  ros::Subscriber WP_permission_command = n.subscribe(robNS+"/WP_permission", 100, WP_permission_callback);

  ros::Subscriber GPS_data = n.subscribe(robNS+"/SENSORS/GPS", 100, GPS_callback);
  ros::Subscriber IMU_data = n.subscribe(robNS+"/SENSORS/IMU", 100, IMU_callback);
  ros::Subscriber SICK_data = n.subscribe(robNS+"/SENSORS/SICK/scan", 100, SICK_callback);
  ...
  wheelsrate_pub = n.advertise<geometry_msgs::Twist>(robNS+"/wheelsrate", 1000);

  mission_status_pub = n.advertise<std_msgs::Int8>(robNS+"/mission_status", 100);

...
int main(int argc, char **argv)
{
	...
	if (argc != 4)
	{
		ROS_ERROR("srvss_bobcat_rate2effort got no Robot Name Space - seting defult: robNS = srvss_bobcat ");
		robNS = "/srvss_bobcat";
	}
  robNS = argv[1];
  ...
  front_left_pub_ = n.advertise<std_msgs::Float64>(robNS+"/front_left_wheel_velocity_controller/command", 100);
  front_right_pub_ = n.advertise<std_msgs::Float64>(robNS+"/front_right_wheel_velocity_controller/command", 100);
  back_left_pub_ = n.advertise<std_msgs::Float64>(robNS+"/back_left_wheel_velocity_controller/command", 100);
  back_right_pub_ = n.advertise<std_msgs::Float64>(robNS+"/back_right_wheel_velocity_controller/command", 100);
...
//  loader_pub_ = n.advertise<std_msgs::Float64>("/bobcat/loader_position_controller/command", 100);
    ...
    ros::Subscriber twist_sub_ = n.subscribe(robNS+"/wheelsrate", 100, wheelsCallback );
    ros::Subscriber JointStates_sub_ = n.subscribe(robNS+"/joint_states", 100, joint_statesCallback );
